...
#
# Rule: 15+ Read|Bash|Grep|Glob calls and 0 Task calls → soft warning, once.
# (Was 30 — dropped because by then the linear cache_read curve is already steep.
# 15 is past the noise floor of normal orientation reads but early enough to
# salvage the rest of the session by dispatching a subagent next.)
#
...
if (( io >= 15 )) && (( tasks == 0 )) && shouldFireOnce "subagent-nudge" "$warnedFile"; then
