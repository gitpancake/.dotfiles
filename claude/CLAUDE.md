...
**Before using `WebFetch`, `WebSearch`, or `context7` to look up API documentation, ALWAYS `find` or `search` OpenViking first.** user has indexed key API docs there — re-fetching wastes time and context. If OV has it, use it. If OV doesn't have it and you fetch it externally, store it in OV with `add_resource` for next time.
