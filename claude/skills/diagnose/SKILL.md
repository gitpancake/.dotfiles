---
name: diagnose
description: 'Diagnosis loop — reproduce → minimise → hypothesise → instrument → fix → regression-test. Trigger: "diagnose"/"debug"/bug report/broken/throwing/failing/perf regression.'
---
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), propose it — a Linear ticket via `/scope`, or `/arch-audit` on the area for a broader sweep. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
