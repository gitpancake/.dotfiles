...
export default function (pi: ExtensionAPI) {
  pi.on("tool_call", async (event, ctx) => {
    if (event.toolName === "bash") {
      ...
      if (isBlockedPrPlaceholder(command)) {
        return {
          block: true,
          reason:
            "Blocked PR write with missing Linear ticket placeholder. Create/reuse a real AE-* ticket, link it in the PR body, and prefix the PR title with [AE-NNNN].",
        };
      }
      return undefined;
    }
    ...
    if (event.toolName === "read") {
      ...
      if (targetPath && isLargeTextFile(targetPath) && ctx.hasUI) {
        ctx.ui.notify(
          `Large read: ${targetPath}. Prefer search then offset/limit.`,
          "warning",
        );
      }
      return undefined;
...
function isBlockedLinearCreate(command: string): boolean {
  ...
  const invocation = /(?:^|[;&|]\s*|\n\s*)(?:[A-Za-z_][A-Za-z0-9_]*=(?:'[^']*'|"[^"]*"|\S+)\s+)*(?:(?:python|python3)\s+)?(?:~?\/?[\w.\/-]+\/)?linear-ticket\.py\s+(?:\\\s*\n\s*|[^;&|\n])*\bcreate\b/;
  return invocation.test(command);
}

function isBlockedPrPlaceholder(command: string): boolean {
  if (!/\bgh\s+pr\s+(create|edit)\b/.test(command)) return false;
  if (command.includes("LINEAR_BYPASS_OK=1")) return false;

  const writesPrTitle =
    /\bgh\s+pr\s+create\b/.test(command) || /--title\b/.test(command);
  if (writesPrTitle && !/\[AE-\d+\]/.test(command)) return true;

  return /\[(ENG|AE)-?(XXXX)?\]|\b(ENG|AE)-XXXX\b|https:\/\/linear\.app\/[^\s"']+\/(ENG|AE)(?:-(?!\d)|(?![-\d]))|Linear ticket\s*\\n\\nN\/A/i.test(
    command,
  );
}
...
function worktreeWriteViolation(
  cwd: string,
  targetPath: string,
): string | undefined {
  ...
  if (isAllowedTicketPath(targetPath)) return undefined;

  return `Blocked cross-worktree write: ${targetPath} is outside current worktree ${git.topLevel}`;
}

function isAllowedTicketPath(targetPath: string): boolean {
  const configuredTicketsDir = process.env.TICKETS_DIR;
  const allowedRoots = [
    configuredTicketsDir,
    resolve(homedir(), ".pi", "agent", "tickets"),
  ].filter((value): value is string => Boolean(value));

  return allowedRoots.some((root) => isInside(targetPath, root));
}

function gitInfo(
  cwd: string,
): { topLevel: string; gitDir: string; gitCommonDir: string } | undefined {
  try {
